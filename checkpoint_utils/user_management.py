    ...
    def user_exists(self, username: str) -> bool:
        ...
        try:
            ...
            if home_dir_pattern in output:
                # User exists - extract user info from the data line
                logger.info(f"✓ User {username} EXISTS - found home directory pattern: {home_dir_pattern}")
                lines = output.strip().split('\n')
                # Find the line with user data (contains the home directory)
                user_info = "User exists"
                # for line in lines:
                #     if home_dir_pattern in line:
                #         user_info = line.strip()
                #         logger.debug(f"Extracted user info line: '{user_info}'")
                #         break
                # logger.info(f"User {username} details: {user_info}")
                return True
